...
def fcn32s_description(features, labels, params, mode, config):
    ...
    conv2_1 = layers.conv_pool_layer(bottom=pool1, filters=128, training=training, name="2_1",
                                     pool=False)
    pool2 = layers.conv_pool_layer(bottom=conv2_1, filters=128, training=training, name="2_2")

    # print("SHAPE Conv_2: ", pool2.shape)

    conv3_1 = layers.conv_pool_layer(bottom=pool2, filters=256, training=training, name="3_1",
                                     pool=False)
    conv3_2 = layers.conv_pool_layer(bottom=conv3_1, filters=256, training=training, name="3_2",
                                     pool=False)
    pool3 = layers.conv_pool_layer(bottom=conv3_2, filters=256, training=training, name="3_3")

    # print("SHAPE Conv_3: ", pool3.shape)

    conv4_1 = layers.conv_pool_layer(bottom=pool3, filters=512, training=training, name="4_1",
                                     pool=False)
    conv4_2 = layers.conv_pool_layer(bottom=conv4_1, filters=512, training=training, name="4_2",
                                     pool=False)
    pool4 = layers.conv_pool_layer(bottom=conv4_2, filters=512, training=training, name="4_3")

    # print("SHAPE Conv_4: ", pool4.shape)

    conv5_1 = layers.conv_pool_layer(bottom=pool4, filters=512, training=training, name="5_1",
                                     pool=False)
    conv5_2 = layers.conv_pool_layer(bottom=conv5_1, filters=512, training=training, name="5_2",
                                     pool=False)
    ...
    up_score = layers.up_conv_layer(score_layer, num_filters=num_classes, kernel_size=64, strides=32,
                                    out_size=height, pad="same", name="uc")
